# R Code for Customer Service Data Analysis
...
# change CSAT and item price to numeric 
...
# check if there are any more NAs
na_rows <- subset(df, is.na(survey_date))
print(na_rows)
tail(na_rows)
...
#check if there are any more NAs
na_rows <- subset(df, is.na(clean_channel_name))
print(na_rows)
...
#check the NAs rows
na_rows <- subset(df, is.na(clean_product_category))
print(na_rows)
...
CSAT
#ggsave("csat_distribution.png", CSAT, width = 10, height = 6)
...
#ggsave("csat_by_product.png", CSAT_product_category, width = 12, height = 6)
...
#ggsave("response_time_dist.png", p4, width = 10, height = 6)

######
# Response time analysis Vs CSAT
...
CSAT_tenure
#ggsave("csat_by_tenure.png", p6, width = 10, height = 6)
...
CSAT_shift
#ggsave("csat_by_shift.png", p7, width = 10, height = 6)
...
response_channel <- ggplot(avg_handling, aes(x = reorder(clean_channel_name, -avg_handling), y = avg_handling, fill = clean_channel_name)) +
  ...
  scale_fill_viridis_d() 
response_channel
#ggsave("handling_time_by_channel.png", p8, width = 12, height = 6)
...
unique(df$Customer_City) # there are some corrupted data in between results
...
cities <- ggplot(city_counts, aes(x = reorder(Customer_City, -n), y = n, fill = Customer_City)) +
  ...
  scale_fill_viridis_d() 
cities
#ggsave("top_cities.png", p9, width = 12, height = 6)
...
performance_by_response <- ggplot(agent_perf, aes(x = reorder(Agent_name, -CSAT_Score), y = Handling_time, , fill = Agent_name)) +
...
# Export results
#write.csv(final_analysis, "customer_sentiment_analysis_results.csv", row.names = FALSE)
#write.csv(sentiment_summary, "sentiment_summary_by_topic.csv", row.names = FALSE)


##########
# Return comprehensive analysis
# Just assign the results to a variable instead of using return()
nlp_results <- list(
  data = final_analysis,
...
# individual components can be accessed like this:
nlp_results$plots$emotions

## OR
# Define a function that will contains all the analysis
analyze_customer_sentiment <- function(df) {
  # [All the previous code would go here]
...
results <- analyze_customer_sentiment(df)


# Machine Learning - CSAT Score Prediction
# =========================================

# checking to see category and sub catefgory columns imported properly
...
# Make predictions
y_pred <- predict(rf_model, X_test_complete)
...
# Load necessary libraries for evaluation and visualization
library(pROC)
library(gridExtra)
...
print(paste("Logistic Regression Accuracy:", round(accuracy_lr, 4)))
...
# Save the ROC plot
#ggsave("roc_comparison.png", roc_plot, width = 8, height = 6, dpi = 300)
...
#ggsave("variable_importance.png", importance_plot, width = 8, height = 10, dpi = 300)
...
-----
str(binned_combined)
colSums(is.na(binned_combined))
nrow(binned_combined)
binned_combined$count <- as.numeric(binned_combined$count)

# Improved binning function with better handling of edge cases
improved_bin_probabilities <- function(prob_df, bins) {
  # Create break points for binning
  breaks <- seq(0, 1, length.out = bins + 1)
  
  # Add a small epsilon to the last break to include 1.0 in the last bin
  breaks[length(breaks)] <- breaks[length(breaks)] + 1e-10
  
  prob_df$bin <- cut(prob_df$predicted_prob, breaks = breaks, include.lowest = TRUE)
  
  # Group and summarize with safeguards against empty bins
  binned <- prob_df %>%
    group_by(bin) %>%
    summarize(
      count = n(),
      ...
      .groups = 'drop'  # This prevents grouping information being retained
    )
  
  return(binned)
}

# Recreate the binned data with improved function
binned_rf <- improved_bin_probabilities(probabilities_rf, bins = 10)
binned_lr <- improved_bin_probabilities(probabilities_lr, bins = 10)
...
# Combine for plotting
improved_binned_combined <- rbind(binned_rf, binned_lr)

# Print the improved binned data
print("Improved binned data:")
print(improved_binned_combined)
...
#ggsave("calibration_plot.png", calibration_plot, width = 8, height = 6, dpi = 300)
...
#ggsave("precision_recall_curve.png", pr_plot, width = 8, height = 6, dpi = 300)
...
#ggsave("model_comparison.png", comparison_plot, width = 10, height = 8, dpi = 300)
...
city_analysis <- df %>%
  ...
  summarise(
    ...
    Avg_Handling_Time = mean(Connected.handling.time, na.rm = TRUE),
...
product_analysis <- df %>%
  ...
  summarise(
    ...
    Avg_CSAT = mean(CSAT.Score, na.rm = TRUE),
    Avg_Handling_Time = mean(Connected.handling.time, na.rm = TRUE),
    Avg_Item_Price = mean(Item.price, na.rm = TRUE)
  ) %>%
  arrange(desc(Ticket_count))
...
price_CSAT <- ggplot(df, aes(x = Item.price, y = CSAT.Score)) +
  ...
  xlim(0, quantile(df$Item.price, 0.95, na.rm = TRUE))  # Remove outliers for better visualization
price_CSAT
#ggsave("price_vs_csat.png", p12, width = 10, height = 6)
...
if ("Issue.reported.at" %in% colnames(df)) {
  df$Day_of_week <- wday(df$Issue.reported.at, label = TRUE)
  df$Hour_of_day <- hour(df$Issue.reported.at)
  ...
  CSAT_weekday <- ggplot(day_csat, aes(x = Day_of_week, y = Avg_CSAT, group = 1)) +
    ...
    theme_minimal()
  CSAT_weekday
  #ggsave("csat_by_day.png", p13, width = 10, height = 6)
  ...
  CSAT_hour
  #ggsave("csat_by_hour.png", p14, width = 12, height = 6)
  ...
  hour_day <- ggplot(hour_day_volume, aes(x = Hour_of_day, y = Day_of_week, fill = n)) +
    ...
    scale_x_continuous(breaks = 0:23)
  hour_day
  #ggsave("ticket_heatmap.png", p15, width = 12, height = 6)
}
...
agent_matrix <- ggplot(significant_agents, 
              aes(x = Avg_Handling_Time, y = Avg_CSAT, size = Ticket_count, label = Agent.name)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_text_repel(size = 3, box.padding = 0.5, segment.alpha = 0.6) +
  labs(title = "Agent Performance Matrix", 
       x = "Average Handling Time", y = "Average CSAT Score", size = "Ticket Count") +
  theme_minimal()
agent_matrix
#ggsave("agent_performance_matrix.png", p16, width = 12, height = 8)
...
sent <- ggplot(df, aes(x = Sentiment_category, y = Remarks_length, fill = Sentiment_category)) +
  ...
  theme(legend.position = "none")
sent
#ggsave("sentiment_vs_length.png", p17, width = 10, height = 6)
...
top_channel <- channel_csat$Channel.name[1]
worst_channel <- tail(channel_csat$Channel.name, 1)[1]
...
avg_response_high_csat <- df %>%
  filter(High_CSAT == 1) %>%
  summarise(avg_time = mean(Response_time_min, na.rm = TRUE)) %>%
  pull(avg_time)
...
# Agent training
if ("Tenure.Bucket" %in% colnames(df)) {
  tenure_csat <- df %>%
    group_by(Tenure.Bucket) %>%
    summarise(avg_csat = mean(CSAT.Score, na.rm = TRUE)) %>%
    arrange(avg_csat)
  
  lowest_tenure <- tenure_csat$Tenure.Bucket[1]
